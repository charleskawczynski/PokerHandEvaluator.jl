...
recurse_up(card::Card) = recurse_up(typeof(rank(card)))
...
type_up(card::Card, n::Int) = type_up(typeof(rank(card)), n)
type_dn(card::Card, n::Int) = type_dn(typeof(rank(card)), n)
...
hand_rank(t::Tuple) = hand_rank((t[5], t[1:4]...))
...
hand_rank(::Tuple{Card{R,S1},Card{R,S2},Card{R,S3},Card{R,S4},Card{RK,S5}}) where {R,RK,S1,S2,S3,S4,S5} =
...
hand_rank(::Tuple{Card{R3,S1},Card{R3,S2},Card{R3,S3},Card{R2,S4},Card{R2,S5}}) where {R3,R2,S1,S2,S3,S4,S5} =
...
hand_rank(::Tuple{Card{R3,S1},Card{R3,S2},Card{R3,S3},Card{R1,S4},Card{R2,S5}}) where {R3,R1,R2,S1,S2,S3,S4,S5} =
